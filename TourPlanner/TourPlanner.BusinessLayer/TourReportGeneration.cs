...
namespace TourPlanner.BusinessLayer
{
    public class TourReportGeneration
    {
        ...
        public void TourReportGenerator(Tour selectedTour, ObservableCollection<Log> logs)
        {
                ...
                container.Page(page =>
                {
                    ...
                    page.Content()
                        .Column(column =>
                        {
                            ...
                            column.Item().AlignLeft().Text("Logs:").SemiBold().FontSize(20).FontColor(Colors.Blue.Medium);
                            column.Item().Table(table =>
                            {
                                ...
                                table.Header(header =>
                                {
                                    ...
                                    header.Cell().BorderBottom(2).BorderColor(Colors.Black).PaddingHorizontal(8).AlignLeft().Text("Comment");
                                    header.Cell().BorderBottom(2).BorderColor(Colors.Black).AlignRight().Text("Difficulty");
                                    header.Cell().BorderBottom(2).BorderColor(Colors.Black).AlignRight().Text("Rating");
                                });
                                foreach (var log in logs)
                                {
                                    table.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).Text(log.StartTime).FontSize(10);
                                    table.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).Text(log.DisplayTime).FontSize(10);
                                    table.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).PaddingHorizontal(8).AlignLeft().Text(log.Comment).FontSize(10);
                                    table.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).AlignRight().Text(log.Difficulty).FontSize(10);
                                    table.Cell().BorderBottom(1).BorderColor(Colors.Grey.Lighten2).AlignRight().Text(log.Rating).FontSize(10);
                                }
                            });
                        });
                    page.Footer()
